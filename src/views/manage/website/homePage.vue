<template>
  <div>
   ...
   <el-table :data="table" border>
     ...
     <el-table-column min-width="10%" label="确认">
       <template slot-scope="scope">
...
export default {
  ...
  methods: {
    initData () {
      this.table = []
      this.$http.post(this.$domain.domain1 + 'electric-design/getHomepagedata').then((res) => {
        this.json1 = res.data
        delete this.json1.code
      }).catch((err) => {
        console.log(err)
      })
      this.$http.post(this.$domain.domain1 + 'electric-design/getDataFormatOfHompage').then((res) => {
        this.json2 = res.data
      }).catch((err) => {
        console.log(err)
      })
    ...
    change (index, row) {
      ...
      this.$http.post(this.$domain.domain1 + 'electric-design/changeHomePage', json).then((res) => {
        if (res.data.result) {
          this.$message({
            type: 'success',
